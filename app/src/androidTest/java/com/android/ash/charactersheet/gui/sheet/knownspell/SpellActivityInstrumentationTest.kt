...
class SpellActivityInstrumentationTest : KoinTest {
    ...
    fun onCreate() {
        ...
        onView(withId(R.id.spell_range)).check(matches(withText("myRange")))
        onView(withId(R.id.spell_effect)).check(matches(withText("Effect: myEffect")))
        onView(withId(R.id.spell_duraction)).check(matches(withText("Duration: myDuration")))
