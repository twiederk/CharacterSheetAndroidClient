...
public abstract class AbstractAndroidDisplayService extends AbstractDisplayService {
    ...
    public String getDisplayWeight(final float weight) {
        ...
        return decimalFormat.format(weight) + " " + resources.getString(R.string.unit_weight_abbreviation);
    }

    @Override
    public String getDisplayCost(final float cost) {
        final DecimalFormat decimalFormat = (DecimalFormat) NumberFormat.getInstance(Locale.US);
        return decimalFormat.format(cost) + " " + resources.getString(R.string.currency_gold_abbreviation);
    }
    ...
    private List<String> getSpellComponentLetters(final Spell spell) {
        final List<String> letters = new ArrayList<>(5);
        if (spell.isVerbal()) {
